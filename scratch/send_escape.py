...
# KeyPress / KeyRelease events structure
...
# Escape keysym is 9 in keycode on standard layouts
keycode = x11.XKeysymToKeycode(display, 0xff1b) # XK_Escape = 0xff1b
print("Escape keycode:", keycode)

def send_key(window, keycode, is_press):
    event = XKeyEvent()
    event.type = 2 if is_press else 3 # KeyPress = 2, KeyRelease = 3
...
# Send Escape KeyPress and KeyRelease
send_key(window, keycode, True)
time.sleep(0.05)
send_key(window, keycode, False)

print("Sent Escape key to window successfully.")
