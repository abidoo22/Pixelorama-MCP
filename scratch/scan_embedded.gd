...
func _init():
	var pck_path = "/home/abido/Downloads/Pixelorama-Linux-64bit/Pixelorama.x86_64"
	var success = ProjectSettings.load_resource_pack(pck_path)
	if not success:
		print("Failed to load embedded PCK from binary: ", pck_path)
		quit(1)
		return
	
	print("Embedded PCK loaded successfully!")
	var files = []
	scan_dir("res://", files)
	...
	for f in files:
		if f.ends_with(".gd") or f.ends_with(".gdc"):
			# Since compiled .gdc scripts are binary, let's just search the filenames first
			if target_phrase in f:
				print("Found in filename: ", f)
			# We can also check if the text files have it
